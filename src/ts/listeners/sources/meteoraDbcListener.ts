...
export class MeteoraDbcListener implements IPoolListener {
  ...
  private async _processParallelSafetyAndLpCheck(poolData: PoolData): Promise<void> {
    ...
    try {
      ...
      console.error(`[MeteoraDBC] Error in parallel processing for pool ${poolData.address}:`, error);
    }
  }
  ...
  private async _processSafetyCheck(poolData: PoolData, wasDelayedLpCheck: boolean = false, lpFoundTimeMs: number = 0): Promise<void> {
    try {
      ...
      console.error(`[MeteoraDBC] Error processing safety check for pool ${poolData.address}:`, error);
    }
  }
}
